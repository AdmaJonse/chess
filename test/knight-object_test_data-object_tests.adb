...
--  placed into Knight.Object_Test_Data.
...
--  begin read only
--  end read only
package body Knight.Object_Test_Data.Object_Tests is
...
--  end read only
    ...
    type Position_Array is array (Positive range 1 .. 3) of Position_Type;
    ...
    Test_Data : Test_Record :=
      (Position      => To_Position ("B1"),
       Valid_Moves   => (To_Position ("A3"), To_Position ("C3"), To_Position ("D2")),
       Invalid_Moves => (To_Position ("B1"), To_Position ("A1"), To_Position ("B3")));

    The_Piece : Knight.Object_Access := Knight.Make (White, Test_Data.Position);
...
--  id:2.2/be5243dac935e519/Get_Paths/1/0/
   ...
   --  knight.ads:56:3:Get_Paths
--  end read only

    pragma Unreferenced (Gnattest_T);

    Test_Position : constant Common_Types.Position_Type := ('E', 5);
    White_Piece   : constant Knight.Object_Access       := Knight.Make (Common_Types.White, Test_Position);
    Black_Piece   : constant Knight.Object_Access       := Knight.Make (Common_Types.Black, Test_Position);
    ...
    for Path of White_Piece.Get_Paths loop
      ...
      AUnit.Assertions.Assert
        (White_Piece.Is_Valid_Move (To => Path (Test_Position)),
         "Path specifies invalid move for White Knight.");
    ...
    for Path of Black_Piece.Get_Paths loop
      ...
      AUnit.Assertions.Assert
        (Black_Piece.Is_Valid_Move (To => Path (Test_Position)),
         "Path specifies invalid move for Black Knight.");
...
--  id:2.2/16ec4bf257e5d477/Image/1/0/
   ...
   --  knight.ads:68:3:Image
--  end read only

      pragma Unreferenced (Gnattest_T);

    White_Piece : constant Knight.Object_Access := Knight.Make (White, To_Position ("A1"));
    Black_Piece : constant Knight.Object_Access := Knight.Make (Black, To_Position ("A2"));

    White_Expected : constant String := (1 => 'N');
    Black_Expected : constant String := (1 => 'n');
...
--  id:2.2/1449071bc3da3a31/Get_Blocked_Squares/1/0/
   ...
   --  knight.ads:81:3:Get_Blocked_Squares
--  end read only
  ...
  begin
    ...
    declare
      ...
      Actual_Squares   : constant Position_Vector.Vector := Game.Get_Board.Get_Square (('D', 4)).Get_Contents.Get_Blocked_Squares;
    begin
...
--  begin read only
--  end read only
end Knight.Object_Test_Data.Object_Tests;
