...
--  placed into King.Object_Test_Data.
...
--  begin read only
--  end read only
package body King.Object_Test_Data.Object_Tests is
...
--  end read only
    ...
    Test_Data : Test_Record :=
      (Position      => To_Position ("A1"),
       Valid_Moves   => (To_Position ("B2"), To_Position ("A2"), To_Position ("B1")),
       Invalid_Moves => (To_Position ("B3"), To_Position ("A3"), To_Position ("C1")));

    The_Piece : King.Object_Access := King.Make (White, Test_Data.Position);
...
--  id:2.2/be5243dac935e519/Get_Paths/1/0/
   ...
   --  king.ads:56:3:Get_Paths
--  end read only

    pragma Unreferenced (Gnattest_T);

    Test_Position : constant Common_Types.Position_Type := ('E', 5);
    White_Piece   : constant King.Object_Access         := King.Make (Common_Types.White, Test_Position);
    Black_Piece   : constant King.Object_Access         := King.Make (Common_Types.Black, Test_Position);
    ...
    for Path of White_Piece.Get_Paths loop
      ...
      AUnit.Assertions.Assert
        (White_Piece.Is_Valid_Move (To => Path (Test_Position)),
         "Path specifies invalid move for White King.");
    ...
    for Path of Black_Piece.Get_Paths loop
      ...
      AUnit.Assertions.Assert
        (Black_Piece.Is_Valid_Move (To => Path (Test_Position)),
         "Path specifies invalid move for Black King.");
...
--  end read only
    ...
    White_Piece : constant King.Object_Access := King.Make (White, To_Position ("A1"));
    Black_Piece : constant King.Object_Access := King.Make (Black, To_Position ("A2"));

    White_Expected : constant String := (1 => 'K');
    Black_Expected : constant String := (1 => 'k');
...
--  id:2.2/1449071bc3da3a31/Get_Blocked_Squares/1/0/
   ...
   --  king.ads:81:3:Get_Blocked_Squares
--  end read only
...
--  begin read only
--  end read only
end King.Object_Test_Data.Object_Tests;
